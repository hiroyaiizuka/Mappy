...
- **切り替え直後の保存とドラッグ中の表示の変更**: 左下でレイアウトを切り替えた直後（マップが読み直される前）に編集やドロップをすると、切り替えの書き込みと重なって保存が「Markdown が変更されています」で拒否されることがあります。その場合はもう一度操作してください。トピックや本体を運んでいる最中に（タッチで別の指を使うなどして）レイアウトを切り替えた場合は、全体表示への調整を離すまで待ちますが、この操作は実機ではまだ確認していません。運んでいる最中にホイールやズーム・全体表示のボタンで表示を動かすと、運んでいる木がポインターからずれます。
