...
- **入力中にタブを閉じる**: ノードの入力欄を開いたまま（Enter も Escape も押さずに）マップのタブや別ウィンドウを閉じると、入力中の内容はノートに保存されます。ただし、保存を拒否されて入力欄にエラーが出ている入力と、日本語入力で変換を確定していない入力は、保存されずに消えます（閉じたときの保存が拒否された場合も、知らせは出ません）。残したい入力は Enter で確定してから、残したくない入力は Escape で取り消してから閉じてください。
...
- **操作の直後の一瞬の表示**: 読み込みの遅い環境では、トピックをノードの隣へ落とす・枝をキャンバスへ切り離す・改名・Cmd/Ctrl+Z などのあとや、トピックをキャンバスに離した直後にレイアウトを切り替えたとき、一瞬だけ操作の前の表示が描かれてから新しい表示になることがあります（ノートに保存される内容は変わりません）。
