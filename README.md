...
| `mappy-topics` | `見出しの文: { mindmap: [x, y], timeline: [x, y] }` | フリートピックの位置。見出しの文をキーに、レイアウト別に保存する |
...
| マップを検索して呼び出す | `mappy: true` の別のノートを検索し、選択ノード（未選択なら本体ルート）の子の末尾に `![[別マップ]]` の項目として追加する。マップがアクティブなときだけ |
...
キャンバスは全面で、レイアウトの切り替えは左下、操作メニュー（歯車）は右上、ズームは右下の浮かせたボタンにあります。

| 操作 | 方法 |
| --- | --- |
| 操作メニュー | 右上の歯車。Markdown に切り替え・左に Markdown を開く／兄弟を追加（Enter）・子を追加（Tab）・トピックを追加／テキストを編集（F2）・本文・リンクを編集・画像を追加・折りたたみ（Space）・削除（Delete）／マップを検索して呼び出す・Excalidraw の図面に挿入・SVG／PNG に書き出し・リスト形式に変更／元に戻す・やり直す。下の表のキーや右クリック、コマンドと同じ結果になり、ノードが要る項目はノートを開いていないと、Excalidraw の項目はプラグインがないと選べない。ここからの「トピックを追加」は本体のそばの既定位置に置く（ドラッグで位置を保存） |
| 選択の移動 | ↑／↓で表示順、←で親、→で最初の子 |
...
| 別のマップを呼び出す | ノードのテキストを `![[マップノート]]` だけにする（「マップを検索して呼び出す」でも同じ。`![[ノート#見出し]]` はその見出しの下だけ）。そのノードが呼び出したマップのルートになり、呼び出したマップの枝が現在のマップの枝と同じ見た目・同じレイアウトで右に並ぶ（開いた直後はルートの子まで）。呼び出した部分は読み取り専用（控えめな文字色、ノードの前に link の印、ホバーで元ノートのパス）で、選択・折りたたみと、ダブルクリックで元のマップを開くことだけができる。`![[…]]` のノード自体は通常どおり編集・移動・削除でき、削除すると呼び出した枝ごと消える。自分自身と呼び出したマップの中の呼び出しはリンクのまま |
...
| 操作 | 方法 |
| --- | --- |
| 追加 | 空白をダブルクリック、または空白の右クリック「トピックを追加」。押した位置に `## ` の区画を作り、その場で入力 |
| 移動 | トピックのルートをドラッグ。位置は `mappy-topics` に保存する |
