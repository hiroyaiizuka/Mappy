...
| 別のマップを呼び出す | ノードのテキストを `![[マップノート]]` だけにする（「マップを検索して呼び出す」でも同じ）。そのノードが読み取り専用の枠になり、枠の中のダブルクリックか右上のボタンで元のマップを開く。自分自身と枠の中の呼び出しはリンクのまま |
...
Obsidian の設定 → Mappy に 3 項目があります。既定値のままなら設定がなかったときと同じ動作で、設定を変えてもノートは書き換えません。
