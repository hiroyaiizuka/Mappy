...
- **Undo の履歴は Markdown エディタと別です。** マップの履歴はノートごとに持ち、Markdown 側や外部からの変更を検出したときと、左下でレイアウトを切り替えてノートの `mappy-layout` を書き換えたときに破棄します（そのとき、切り替えより前の操作は Cmd/Ctrl+Z で戻せず、取り消した操作をやり直すこともできません）。Markdown 側の Cmd/Ctrl+Z はエディタの履歴で動きます。
...
- **切り替えと同時の保存（0.3.5 まで）**: テキストの入力中に左下のボタンでレイアウトを切り替えると、入力の確定が切り替えの書き込みと重なって拒否されることがあります。切り替えの直後（マップが読み直される前）のキー操作・ドロップ・画像の貼り付けも同じ理由で拒否されえます（こちらは人の操作で起きるかを確かめていません）。拒否されると「Markdown が変更されています」「Markdown が更新されました…」「ノートが更新されました…」のどれかが出るので、もう一度操作してください。また、同じ名前のノードや題名が空のノードは、切り替えで折りたたみと選択が外れます。どちらも次の版で直ります。
- **ドラッグ中の切り替えと表示の変更**: トピックや本体を運んでいる最中に（タッチで別の指を使うなどして）左下でレイアウトを切り替えた場合は、全体表示への調整を離すまで待ち、次の版からは離したときの保存も切り替えと重なって拒否されませんが、どちらもタッチ画面の端末ではまだ確認していません。運んでいる最中にホイールやズーム・全体表示のボタンで表示を動かすと、運んでいる木がポインターからずれることがあります（これも実機では未確認です）。
...
| 操作を取り消したい | マップ上で Cmd/Ctrl+Z。外部変更やレイアウトの切り替えで履歴が消えた場合は、そのノートを Markdown エディタでも開いていればエディタの Cmd/Ctrl+Z で戻せることがある。ほかはコアプラグインの「ファイルリカバリー」やバックアップから戻す |
