...
- **操作の直後の一瞬の表示（0.3.7 まで）**: 読み込みの遅い環境では、トピックをノードの隣へ落とす・枝をキャンバスへ切り離す・改名・Cmd/Ctrl+Z などのあと、一瞬だけ操作の前の表示が描かれてから新しい表示になることがあります（ノートに保存される内容は変わりません）。トピックをキャンバスの空いた場所に離しただけならこの表示は出ませんが、離した直後にレイアウトを切り替えた場合は出ることがあります。
