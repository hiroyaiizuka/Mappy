...
- **Undo の履歴は Markdown エディタと別です。** マップの履歴はノートごとに持ち、Markdown 側や外部からの変更を検出したときと、左下でレイアウトを切り替えてノートの `mappy-layout` を書き換えたときに破棄します（そのとき、切り替えより前の操作は Cmd/Ctrl+Z で戻せません）。Markdown 側の Cmd/Ctrl+Z はエディタの履歴で動きます。
...
- **切り替え直後の保存（0.3.5 まで）**: 0.3.5 までは、左下でレイアウトを切り替えた直後（マップが読み直される前）に編集・ドロップ・画像の貼り付けをすると、切り替えの書き込みと重なって保存が「Markdown が変更されています」で拒否されることがあります。拒否されたら、もう一度操作してください。次の版で直ります（指で運んだトピックや本体を切り替えの直後に離す場合は、タッチ画面の端末ではまだ確認していません）。
- **ドラッグ中の表示の変更**: トピックや本体を運んでいる最中に（タッチで別の指を使うなどして）左下でレイアウトを切り替えた場合は、全体表示への調整を離すまで待ちますが、この操作はタッチ画面の端末ではまだ確認していません。運んでいる最中にホイールやズーム・全体表示のボタンで表示を動かすと、運んでいる木がポインターからずれることがあります（これも実機では未確認です）。
...
| 操作を取り消したい | マップ上で Cmd/Ctrl+Z。外部変更やレイアウトの切り替えで履歴が消えた場合はコアプラグインの「ファイルリカバリー」やバックアップから戻す |
