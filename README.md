...
ノード内の入力欄は Mappy 専用で、Obsidian の標準エディタではありません。候補にはノートの名前・パス・別名に加え、PNG・SVG・PDF など Vault 内の添付ファイルも出ます。
...
- **F2**: Obsidian の既定ホットキー「ファイル名を編集（Edit file title）」と重なり、環境によってはマップの編集が開きません。ダブルクリックを使うか、Obsidian 側のホットキーを変えてください。
