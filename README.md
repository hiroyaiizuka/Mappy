...
- **操作の直後の一瞬の表示**: 読み込みの遅い環境では、トピックをノードの隣へ落とす・枝をキャンバスへ切り離す・改名・Cmd/Ctrl+Z などのあと、一瞬だけ操作の前の表示が描かれてから新しい表示になることがあります（ノートに保存される内容は変わりません）。
