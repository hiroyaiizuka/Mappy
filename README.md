...
| 選択の移動 | ↑／↓で表示順、←で親、→で最初の子。空白をクリックすると選択が外れ（背景のドラッグでは外れない）、矢印キーで本体ルートから選び直す |
...
| 操作 | 方法 |
| --- | --- |
| 追加 | 空白をダブルクリック、または空白の右クリック「トピックを追加」。押した位置に `## ` の区画を作り、その場で入力。何も選択せずに「マップを検索して呼び出す」を実行すると `## ![[別マップ]]` のトピックが本体のそばに置かれる |
