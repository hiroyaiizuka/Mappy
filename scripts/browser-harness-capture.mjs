...
/** docs/harness.md E31 on this page: a note that embeds maps, in the reading-view path and the live-preview path. */
async function captureEmbeds(recorder, page) {
  ...
  await recorder.run('embed-dark', '暗色の仮テーマに切り替える', 'CSS 変数だけの差し替えで枠・ノード・線が見え、崩れない（Obsidian のテーマそのものではない）', async () => {
    await page.harness('h.setTheme(true)');
  ...
  await recorder.run('embed-dark-hierarchy', '暗色のまま階層図の埋め込みへスクロール', '線・枠・件数バッジが暗色でも読める', async () => {
    ...
    await page.harness('h.setTheme(false)');
  });
...
function recordMarkdown({ startedAt, chrome, version, commit, cases, timings, notExecuted }) {
  const lines = [
    ...
    '- 対象外: 保存、リンク解決、テーマ、日本語 IME。ここでの PASS は Obsidian 実機（③ E01〜E31）の PASS ではない。',
...
async function main() {
  ...
  const notExecuted = [
    'Obsidian 実機（③ E01〜E31）: このページは代替ではない。実機の確認は実機を使うチケットの記録に残す。',
    ...
    '埋め込み（E31）の実機: Obsidian の閲覧モード・ライブプレビュー・ホバープレビューでの描画、テーマ、埋め込み内リンクの遷移、Mappy 無効化での復帰は、このページの閲覧モード相当／ライブプレビュー相当のケース（embed-*）では確認できない。',
  ];
