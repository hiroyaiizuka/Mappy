...
async function captureSameTitledTopics(recorder, page) {
  ...
  await recorder.run('topic-same-heading-drag', 'free-topics に `## 同じ見出し` の区画を 2 つ足して開く → 2 つ目の「同じ見出し」を右下へ 120×60 px ドラッグ',
    ...
    expect(firstAfter && Math.abs(firstAfter.rect.x - first.rect.x) < 1.5 && Math.abs(firstAfter.rect.y - first.rect.y) < 1.5,
      `the first topic moved on screen by ${(firstAfter.rect.x - first.rect.x).toFixed(1)}, ${(firstAfter.rect.y - first.rect.y).toFixed(1)}`);
    expect(secondAfter && Math.abs(secondAfter.rect.x - second.rect.x - 120) < 1.5 && Math.abs(secondAfter.rect.y - second.rect.y - 60) < 1.5,
      `the second topic moved by ${(secondAfter.rect.x - second.rect.x).toFixed(1)}, ${(secondAfter.rect.y - second.rect.y).toFixed(1)}`);
