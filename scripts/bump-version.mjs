...
/**
 * Point manifest.json at `version` and record its minAppVersion in versions.json.
 * package.json and package-lock.json are npm's job (`npm version`); validate-release
 * catches any drift between the four files afterwards.
 */
...
if (invokedAsScript) {
  // `npm version x.y.z` runs this as the `version` script after updating package.json
  // and package-lock.json, passing the new version through the environment.
