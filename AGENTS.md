...
- ミスをしたら `corrections/inbox` に `edit-note --operation append` で追記する。**存在しない permalink へ `append` するとエラーにならず、`mappy-memory/` を前置した別のノートが黙って作られる**ので、先に `bm tool read-note corrections/inbox --project mappy-memory` で在ることを確かめ、無ければ `write-note` で作る。そこから先の流れ（inbox → lessons → graduated）は `memory/schemas/correction.md` が正本で、このファイルにもスキルにも写さない（`bm tool read-note schemas/correction --project mappy-memory` で読む）。
