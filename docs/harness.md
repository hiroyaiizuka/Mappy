...
| E35 | `Fixtures/embed-nodes.md` をマップで開く（`![[embed-timeline]]`・`![[embed-hierarchy#同じ名前]]`・`![[embed-2000]]`・同じ `![[embed-timeline]]` の 2 回目・`![[embed-cycle]]` だけのノードを持つ）→ 枠の中の開閉ボタン → 枠の中をダブルクリック → 右上の「マップで開く」→ 別 leaf で `embed-timeline` を編集（保存前・保存後）→ `![[embed-timeline]]` のノードで F2・Enter・Delete・ドラッグ並べ替え → `embed-cycle` を開く → view を閉じる → Mappy を無効化 | 5 つのノードが読み取り専用のマップの枠（ルートと第一階層、以下は折りたたみ、Fit で拡大なし）になり、自分自身・文中の埋め込み・`mappy: true` のないノート・存在しないノート・ブロック参照はリンク、画像は画像のまま。明色・暗色テーマで枠・線・件数バッジが読める。枠内の開閉は枠の中だけで原文を変えず、枠内のダブルクリックと「マップで開く」で元ノートがマップで開く（§8 のルーティング）。別 leaf の編集（未保存でも）で 2 つのタイムラインの枠が更新され、ホストと元ノートの原文は変わらない。F2 は原文 `![[…]]` をそのまま編集し、Enter／Delete／ドラッグは通常のノードと同じ差分で、枠の中のノードを掴まない。ホイール・ドラッグは外側のマップに渡る。`embed-cycle` では `![[embed-nodes]]` が枠になり、その中の `![[embed-cycle]]` はリンク（枠の中に枠はない）で描画が止まらない。view を閉じたあと DOM・イベントが残らず（DevTools のメモリ／`app.workspace`・`app.vault` のリスナー）、Mappy の無効化で残らない（実機は未実施。LEV-72 で行う） |
