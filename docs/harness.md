...
jsdom を導入し、製品の DOM 操作・インライン入力・キー操作を検証している。composition イベントを送るテストは変換中のキー制御を検証するもので、OS の日本語 IME で入力した結果ではない。Obsidian のキーマップ（`window` の capture 段階で active view の `Scope` → グローバルホットキーの順）は jsdom で再現できないので、map view が `Scope`（親 `app.scope`）に修飾キーなしの F2 だけを登録していること、そのハンドラがキャンバス内の選択ノードでは inline 編集を開いて `false`（Obsidian の「消費」）を返し、inline 入力の中・キャンバス外・変換中・選択なしでは `undefined` を返すこと、キーマップ相当のリスナーが `false` で `preventDefault`＋`stopPropagation` してもキャンバスのリスナーが二重に編集を開かないこと（`tests/ui/mindmap-view-scope.test.ts`、`tests/ui/map-events.test.ts`）を固定し、実キーは ③ の CDP で確認する。DOM の実測サイズ、フォント、画像遅延、ポインター操作、ズームの見た目は、下記②のブラウザ検証ページと③の Obsidian 実機で確認する。
