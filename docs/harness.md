...
| E02 | map で Enter/Tab/F2/Delete | フォーカス範囲内だけに作用し本文・子孫を保持。Delete／Backspace／「枝を削除」はリストの項目の行を改行ごと消し、リストの中に空行を残さない（loose list の継ぎ目の空行は 1 つ、末尾の項目は前の改行を取ってファイル末尾の改行の有無を変えない。ブラウザ検証ページで確認済み: `artifacts/lev-75-delete-newline/record.md`。実機は未実施） |
