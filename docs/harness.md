...
| E05 | 外部更新と map 編集を競合させる | 古い状態で上書きせず入力も復旧可能。保持した入力はマップの自動更新後にもう一度 Enter すれば更新後の文書に適用される。編集中のタイトル／本文そのものが外部で変わっていた場合、同名ノード、消えたノードでは拒否されて入力が残る（同じタイトル・本文のまま位置だけ変わったノードには適用する） |
