...
| E16 | 全面キャンバス・浮かせた UI・右クリックの操作 | 操作専用の帯を割かず、左下はレイアウト、右上は歯車 1 つの操作メニュー（Markdown 切り替え・分割／兄弟・子・トピックの追加／テキスト・本文の編集・画像・折りたたみ・削除／呼び出し・Excalidraw・書き出し・リスト形式／元に戻す・やり直す。ボタンの下に右揃えで開き、項目名にキーを併記、ノードが要る項目はノートなしで無効、Excalidraw がなければその項目が無効、各項目はキー・右クリック・コマンドと同じ差分・履歴）、右下はズーム |
