...
| E26 | タイムライン／階層図／左右バランスを選択→Markdown へ切替→マップへ戻る | `mappy-layout: timeline`／`hierarchy`／`balanced` が保存され、再度そのレイアウトで開く。通常マップ選択では任意キーだけを削除する。左右バランス版: 第一階層が原文順に右・左・右・左に分かれ、左側の枝の開閉ボタンと折りたたみバッジが左に出て、本文は変わらない（実機は未実施。LEV-61 で行う） |
...
| E30 | 階層図のマップをアクティブにして「現在のマップを Excalidraw の図面に挿入」 | 挿入した要素が 1 グループで、ルートが上・第一階層が同じ段・線が直角の折れ線。E27 の外枠透明化とは別のケース。左右バランス版: ルートが中央、第一階層が右・左に分かれ、左側の線がルートの左辺から子の右辺への直角の折れ線になり、要素の位置がマップ表示と一致する（実機は未実施。LEV-61 で行う） |
