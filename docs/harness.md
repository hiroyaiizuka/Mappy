...
- マップの中の呼び出し（M12 の表示側）: core で「項目の最初の行が `![[…]]` 1 つだけか」（`embedOnlyTitle`: 前後の空白、`|別名` の切り捨て、文中・2 つ・コード・空は null）。jsdom で製品の view を起動し（`tests/ui/embed-node.test.ts`）、`![[Map]]`／`![[Timeline]]`／`![[Headings#同じ名前]]` のノードが M10 と同じ枠（`data-mappy-embed`、ルートと第一階層、以下は折りたたみ、読み取り専用）になり、自分自身・文中の埋め込み・`mappy: true` のないノート・存在しないノート・ブロック参照・インラインコードはリンク／文、画像は画像のままであること、同じマップの 2 回の呼び出しがそれぞれ独立に開閉すること、A→B→A・A→B→C→A で枠の中の `![[…]]` がリンクになり枠の中に枠がないこと（B 側から開いても同じ）、枠内のクリックで枠を持つノードが選択され枠の中では選択されないこと、枠内の開閉ボタンが枠の中だけを開き原文を変えないこと、外側の開閉で枠がノードごと消えること、枠内のダブルクリック（⌘ で新しいペイン）と右上のボタンが元ノートを開く要求になりインライン入力が開かないこと、F2 とノードの枠外のダブルクリックが原文 `![[…]]` をそのまま編集し取り消しで同じ枠が戻ること、枠内からのドラッグが枠を持つノードのドラッグになりホイールが外側をパンして枠内の transform が変わらないこと、枠内の右クリックが枠を持つノードのメニューになること、Enter／Tab／Delete／改名が通常の差分になり改名で別のマップやただの文に変わること、元ノートの保存・`editor-change` で枠が再描画されホストが変わらないこと、`mappy: true` が外れると枠に一文が出ること、呼び出し先が後から作られる・`mappy: true` を得る・消える（metadataCache の `changed`／`deleted`）とホストを編集しなくてもノードが枠になり／リンクに戻ること、枠のダブルクリックが枠の中のボタンとリンクでは開かないこと（クリックが既に開いている）、枠内の開閉ボタンのクリックが枠を持つノードの選択とフォーカスにもなること（キーボードが外側に残る）、折りたたみ・削除・view の閉じで vault／workspace の購読が減り閉じたあと 0 になること。SVG 書き出し（`svg-capture.test.ts`）は枠を同じ大きさの箱（`mappy-export-embed`、文言はマップ名）として出し、枠の中のノード・線・キャンバスを一切書かないこと。`nodeOf`／`mapClick`（`map-events.test.ts`）と `NodeDrag`（`node-drag.test.ts`）は、枠の中のノード（外側と同じ id でも）が外側のノードとして答えることを固定する。`NodeRenderer`（`node-renderer.test.ts`）は resolver の呼び出し（埋め込み 1 つだけの題名にだけ、原文パスとともに）、mount の相手（entry の Component と content の中の枠）、key に解決先が入り解決先が変わると描き直すこと、resolver なしではリンクのままであることを固定する。
...
- マップの中の呼び出し（M12、E35）: `embed-nodes`／`embed-cycle` は map view の fixture で、`![[…]]` だけのノードが枠になる。`nodes()`／`node()` は view 自身のノードだけを返し（枠の中のノードは含めない。枠を持つノードは `embed: true`、`title` は原文 `![[…]]`）、`nodeEmbeds()` が枠を持つノードとその枠（`embeds()` と同じ形: src・レイアウト・枠内のノード・scale・文言）を DOM 順で返す。`nodes()` の各項目は `link`（自分の label の内部リンク）と `image`（自分の label の画像）も持つ。`listeners()` はページ内 vault／workspace の購読数で、枠の Component の解放を数える。`putNote()` は新しいノートも作り、`removeNote()` はノートを消す（どちらも cache の `changed`／`deleted` を出す）。ダブルクリックと「マップで開く」はリンク解決の要求の記録（`activity`）だけで、実際の遷移は③。
...
このページでの成功を Obsidian の保存・リンク解決・テーマでの成功と呼ばない。Obsidian 実機の確認（下記 E01〜E35）が E2E に相当し、このページはその代替ではない。
...
| E02 | map で Enter/Tab/F2/Delete | フォーカス範囲内だけに作用し本文・子孫を保持。Delete／Backspace／「枝を削除」はリストの項目の行を改行ごと消し、リストの中に空行を残さない（次の兄弟との継ぎ目の空行は一緒に消え、loose list の継ぎ目の空行は 1 つ、末尾の項目は前の改行を取ってファイル末尾の改行の有無を変えない。ブラウザ検証ページで確認済み: `artifacts/lev-75-delete-newline/record.md`。実機は未実施） |
...
| E11 | モバイルと明暗テーマ | タッチ、フォーカス、文字・リンク・画像を操作可能。右上の歯車のポップオーバーがタッチで開いて 3 項目を選べ、明暗テーマで読める |
...
| E35 | 入力側（LEV-70）: `Fixtures/free-topics.md` をマップで開き、何も選択せずにコマンド「マップを検索して呼び出す」→ 候補を見る → `uneven` と打って絞り込む → Enter。「回復する」を選択して同じコマンドで `embed-timeline` を選ぶ。「回復する」の開閉ボタンで枝を閉じてから（足した `![[embed-timeline]]` の枠がトピックのルートを覆うため。LEV-79）「補足: 用語」（トピック）を選択して同じマップをもう 1 度、枝を開き直す。⌘Z を 3 回。設定「新しいリンクの形式」を相対／絶対にして再度呼び出す。表示側（LEV-69）: 次のとおり。`Fixtures/embed-nodes.md` をマップで開く（`![[embed-timeline]]`・`![[embed-hierarchy#同じ名前]]`・`![[embed-2000]]`・同じ `![[embed-timeline]]` の 2 回目・`![[embed-cycle]]` だけのノードを持つ）→ 枠の中の開閉ボタン → 枠の中をダブルクリック → 右上の「マップで開く」→ 別 leaf で `embed-timeline` を編集（保存前・保存後）→ `![[embed-timeline]]` のノードで F2・Enter・Delete・ドラッグ並べ替え → `embed-cycle` を開く → view を閉じる → Mappy を無効化 | コマンドは map view がアクティブなときだけパレットに出る（Markdown 側では出ない）。候補は `mappy: true` のノートだけ（`embed-host`・`heading-document`・自分自身 `free-topics` は出ない）で、ファイル名と親フォルダ `Fixtures` が並び、一致した文字が強調される。原文は `- 習慣化する` の次の行に `- ![[uneven-branches]]`、`  - 睡眠` の次に `  - ![[embed-timeline]]`、`- 用語 B` の次に `- ![[embed-timeline]]` が 1 行ずつ足され、他の行と frontmatter は不変。呼び出したノート（`uneven-branches`・`embed-timeline`）は不変。追加した項目が選択され、インライン入力は開かない。追加した項目は表示側の枠（下記）として描かれる。⌘Z 1 回ごとに 1 項目が丸ごと消えて 3 回で元の原文に戻る。「Wikilinks を使用」がオフでも `![[…]]` のまま、「新しいリンクの形式」の絶対では `![[Fixtures/embed-timeline]]`、相対では同じフォルダなら `![[embed-timeline]]`（サブフォルダなら `![[サブフォルダ/embed-timeline]]`）、最短で同名ノートが 2 つあれば `![[Fixtures/embed-timeline]]` の完全パスになる。一致箇所の強調は Obsidian の `renderMatches` で、`prepareFuzzySearch` が `フォルダ/タイトル` に返す `matches` と同じ位置（実機: LEV-71、`artifacts/lev-71-map-search-e2e/call-map-probe.mjs` と `record.md`。本人の試用で原文が変わっている `free-topics` は使わず、fixture の原文を別名で置いて行う。map view は常に 1 ノードを選択状態に保つので「何も選択せず」は開いた直後の本体ルートが選ばれた状態。ユーザー割り当てのホットキー、同じノートを左に Markdown で開いた分割表示（editor の transaction 経由）、保存中と H2 のないノートの Notice、見出し形式の `### ![[…]]` も同じ probe で確認。候補 0 件の「マップがありません」とモバイルは実機未実施）。表示側: 5 つのノードが読み取り専用のマップの枠（ルートと第一階層、以下は折りたたみ、Fit で拡大なし）になり、自分自身・文中の埋め込み・`mappy: true` のないノート・存在しないノート・ブロック参照はリンク、画像は画像のまま。明色・暗色テーマで枠・線・件数バッジが読める。枠内の開閉は枠の中だけで原文を変えず、枠内のダブルクリックと「マップで開く」で元ノートがマップで開く（§8 のルーティング）。別 leaf の編集（未保存でも）で 2 つのタイムラインの枠が更新され、ホストと元ノートの原文は変わらない。F2 は原文 `![[…]]` をそのまま編集し、Enter／Delete／ドラッグは通常のノードと同じ差分で、枠の中のノードを掴まない。ホイール・ドラッグは外側のマップに渡る。`embed-cycle` では `![[embed-nodes]]` が枠になり、その中の `![[embed-cycle]]` はリンク（枠の中に枠はない）で描画が止まらない。view を閉じたあと DOM・イベントが残らず（`app.vault`／`app.workspace`／`app.metadataCache` の購読数が開く前と同数、枠の要素が GC で回収される）、Mappy の無効化で残らない（実機: LEV-72、`artifacts/lev-72-embed-node-e2e/embed-node-probe.mjs` と `record.md`。ダブルクリック・「マップで開く」は map view が `navigation: false` のため新しいタブに開く＝LEV-74） |
