...
独立したブラウザ検証画面は未実装。追加する場合は製品の core / layout / interaction を読み込んで同じ実装を動かす。モックで成功することを Obsidian の保存・リンク解決・テーマでの成功と呼ばない。
...
| E21 | `mappy-layout` を持つノートを開く／設定・解除コマンド | 既定でマップとして開く。解除で Markdown。frontmatter 以外の本文は変わらない |
...
| E24 | Excalidraw の対話フレームに `mappy-layout` 付きノートを挿入 | Mappy のビューがライブ表示され、線・枠・リンクが見える |
