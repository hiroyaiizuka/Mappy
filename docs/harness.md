...
| E16 | 全面キャンバス・浮かせた UI・右クリックの操作 | 操作専用の帯を割かず、左下はレイアウト、右上は歯車 1 つのポップオーバー（Markdown に切り替え／マップを検索して呼び出す／書き出す の 3 項目。歯車の直下に右辺を揃えたカードで開き、幅 400px のペインでも左辺がペインの左端を越えない。開いた直後は 1 項目目にフォーカス、↑↓で移動、Enter／Space で実行、Escape・外側のクリック・もう一度の押下・view を閉じる で閉じてキャンバスにフォーカスが戻る。ノートなしでは全項目無効。各項目はコマンドと同じ経路。右クリックメニューは変えない）、右下はズーム |
