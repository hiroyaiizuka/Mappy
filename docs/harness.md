...
- メモ: 末尾フェンスの認識、コード・コメント・リスト・引用内や末尾以外の類似記法の除外、H2＋リスト形式と従来形式での一致、追加・文章変更・移動・削除で他範囲と他 frontmatter キーのバイト保全、Obsidian が書き直す block 形式 YAML の読み取り。
...
Obsidian に依存しないブラウザ検証ページは未実装。追加する場合は製品の core / layout / interaction を読み込んで同じ実装を動かし、10／100／500／2,000 ノードの描画・入力反映時間を記録する場とする。モックで成功することを Obsidian の保存・リンク解決・テーマでの成功と呼ばない。Obsidian 実機の確認（下記 E01〜E29）が E2E に相当し、このページはその代替ではない。
...
| E29 | 付箋メモ付きノートを Mappy 以外（通常の Markdown 編集・閲覧モード、Mappy 無効）で開く | メモはコードブロックとして壊れずに表示され、ノード・本文・他の frontmatter が変わらない。Properties に `mappy-memos` が表示されても壊れない |
