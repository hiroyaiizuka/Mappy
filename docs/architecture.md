...
- **入力は view の最終配置**: `MindmapView.exportSource()` は、最後の配置フレームで使った `LayoutResult`、`NodeRenderer.entries` の複製、canvas、線の `<svg>` を渡す。debounce 中の refresh があれば先に実行し、`NodeRenderer.idle()`（表示中のノードの `MarkdownRenderer.render` がすべて終わるまで。上限 `EXPORT_RENDER_WAIT_MS` = 2 秒）を待ち、配置フレームが予約中なら 1 フレーム（隠れたウィンドウでは 100 ms）待ち、インライン編集中・ドラッグ中は拒否する。配置し直さないので、線の `d`・ノードの座標・折りたたみは画面と一致する。
- **描画の完了**（LEV-65）: `NodeRenderer` は進行中の描画をノード id ごとに持ち（値は描画の通し番号。再描画や削除で置き換わった古い描画は、遅れて終わっても待たない）、描画の then／catch で `changed()`（配置フレームの予約）を呼んでから `idle()` の待ち手を起こすので、`exportSource()` は idle の直後に予約済みのフレームを見つけて待てる。画像の `load` は待たない（外部 URL は届かないことがある）。描画が 2 秒を超えて終わらない（post-processor や埋め込みの停止）ときは、map view 自体もその途中の見た目なので、書き出しも画面のまま続行する（エラーにしない）。
