...
フリートピックのルート（view の `projectMap` が topics に入れたノード）は「自由に動くノード」として別扱いにする（`NodeDragActions.free`）。ゴーストも挿入位置の判定も作らず、押下からの移動量（screen px）を view に渡し（`shift`）、view は `LayoutResult.origin` 基準の開始位置＋移動量／倍率を `topicLayouts` の一時的な位置にして毎フレーム再配置するので、トピックの木全体（子・線・開閉ボタン）がポインターに追従する。キャンバス内で離すと `place` → `planTopicMove` で `mappy-topics` のそのレイアウトの項目だけを書く（位置未設定なら項目を作る。Markdown 側の改名で位置を失ったトピックは既定配置から動かした時点で新しいキーが書かれ、旧キーは孤児として残る）。Escape・pointercancel・キャンバス外で離した場合は一時的な位置を捨てて元へ戻し、原文は変えない。ノードの上へ離した場合も現状は位置の更新であり、区画→リストの枝への合流（M7 の残項目）はまだ行わない。
