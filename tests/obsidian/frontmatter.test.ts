...
import {
  LAYOUT_KEY, MAPPY_KEY, MEMOS_KEY, isMappyCandidate, layoutFromFrontmatter, readMapLayout,
...
describe('writeMapLayout', () => {
  ...
  it('removes every Mappy property, memo positions included, without changing unrelated frontmatter', async () => {
    const { instance, store } = app({ tags: ['a'], [MAPPY_KEY]: true, [LAYOUT_KEY]: 'timeline', [MEMOS_KEY]: { m1: { mindmap: [1, 2] } } });
    await writeMapLayout(instance, file(), null);
    expect(store).toEqual({ tags: ['a'] });
  });

  it('keeps memo positions when enabling or changing the layout', async () => {
    const { instance, store } = app({ [MAPPY_KEY]: true, [MEMOS_KEY]: { m1: { mindmap: [1, 2] } } });
    await writeMapLayout(instance, file(), 'timeline');
    expect(store).toEqual({ [MAPPY_KEY]: true, [MEMOS_KEY]: { m1: { mindmap: [1, 2] } }, [LAYOUT_KEY]: 'timeline' });
  });
});
