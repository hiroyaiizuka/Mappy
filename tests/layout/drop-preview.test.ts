...
describe("drop preview tree", () => {
  ...
  it("puts the placeholder before the sibling that would follow the moved node", () => {
    const tree = previewTree(root, { type: "move", nodeId: find(doc, "A3").id, parentId: find(doc, "A").id, index: 0 }, new Set());
  ...
  it("appends after the last other sibling, leaving the moving node in place", () => {
    const tree = previewTree(root, { type: "move", nodeId: find(doc, "A1").id, parentId: find(doc, "A").id, index: 2 }, new Set());
    const a = tree?.children.find(child => child.id === find(doc, "A").id);
    expect(a && titles(doc, a)).toEqual(["A1", "A2", "A3", "▢"]);
  });
  ...
  it("makes the placeholder the only child of a childless destination and reuses untouched subtrees", () => {
    const b = find(doc, "B");
    const tree = previewTree(root, { type: "move", nodeId: find(doc, "A2").id, parentId: b.id, index: 0 }, new Set());
  ...
  it("shows only the placeholder under a collapsed destination", () => {
    const a = find(doc, "A");
    const tree = previewTree(root, { type: "move", nodeId: find(doc, "B").id, parentId: a.id, index: 3 }, new Set([a.id]));
  ...
  it("returns null when the destination is outside the visual root or unknown", () => {
    expect(previewTree(root, { type: "move", nodeId: find(doc, "B").id, parentId: "root", index: 0 }, new Set())).toBeNull();
    expect(previewTree(root, { type: "move", nodeId: find(doc, "B").id, parentId: "missing", index: 0 }, new Set())).toBeNull();
  });
  ...
  it("lays out a rightward map where siblings make room for the placeholder", () => {
    ...
    const tree = previewTree(root, { type: "move", nodeId: find(doc, "B").id, parentId: find(doc, "A").id, index: 1 }, new Set());
