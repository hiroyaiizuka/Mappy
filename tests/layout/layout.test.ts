...
describe("tree layout", () => {
  ...
  it.each(["mindmap", "timeline", "issue-tree"] as const)("removes collapsed descendants and their edges in %s", (mode) => {
  ...
  it.each(["mindmap", "timeline", "issue-tree"] as const)("handles 500 nodes of mixed sizes without overlap in %s", (mode) => {
...
describe("free topics", () => {
  ...
  it.each(["mindmap", "timeline", "issue-tree"] as const)("leaves the body geometry untouched and reports the body root as origin in %s", mode => {
    ...
    // Map: root on the left edge. Timeline: axis through y = 0. Issue tree: root centered on x = 0.
    expect(mode === "timeline" ? root.y + root.height / 2 : mode === "issue-tree" ? root.x + root.width / 2 : root.x).toBe(0);
  });

  it.each(["mindmap", "timeline", "issue-tree"] as const)("stacks unpositioned topics below the body without overlapping anything and inside the bounds in %s", mode => {
    const alone = layoutTree(body, bodySizes, new Set(), mode);
    ...
    for (const id of ["t1", "t2", "t3"]) {
      ...
      // The topic's tree is flush with the body's left edge (its root, except in the issue tree where the root is centered).
  ...
  it.each(["mindmap", "timeline", "issue-tree"] as const)("puts a positioned topic's root at origin + offset, keeps its tree shape, and includes it in the bounds in %s", mode => {
    const placed = [
