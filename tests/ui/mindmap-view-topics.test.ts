...
describe('MindmapView snaps a dragged topic to the slot beside its root', () => {
  ...
  it('on the timeline a short stage beside a tall one takes the topic where its first child lands: past the tall stage\'s half, not its own', async () => {
    // jsdom measures every node as 0 × 0 (so 160 × 44); the first stage 回復する measures 200 tall here, as one with an image would.
    const measured = Object.getOwnPropertyDescriptor(HTMLElement.prototype, 'offsetHeight');
    if (!measured) throw new Error('jsdom defines offsetHeight on HTMLElement.prototype');
    Object.defineProperty(HTMLElement.prototype, 'offsetHeight', {
      configurable: true,
      get(this: HTMLElement) { return this.classList.contains('mappy-node') && this.querySelector('.mappy-node-label')?.textContent === '回復する' ? 200 : 0; },
    });
    try {
      const { view, topic } = await mount(THREE_STAGES, 'timeline');
      const doc = documentOf(view);
      ...
      // Just past the stage itself still counts (the zone reaches back to the stage); the wrong side of the axis is another stage's.
      expect(snap(glossary.id, at(view, stageSpot(lower, 'below')), null)).toEqual({ type: 'move', nodeId: glossary.id, parentId: record.id, index: 0 });
      expect(snap(glossary.id, at(view, landing(lower, 'above')), null)?.parentId).not.toBe(record.id);
