...
function renderInline(escaped: string, app: RendererApp, sourcePath: string, references: Map<string, string>): string {
  ...
  return withoutCode
    ...
    // Obsidian keeps a `<br>` written in the text (a line break inside a node, LEV-202) as core/title-breaks reads it:
    // outside code, not after an escaping backslash (an odd run of them), and not in a wiki link, which this page has
    // already made an internal-link `<a>` of. A Markdown link's label keeps its break.
    .replace(/(<a class="internal-link[^>]*>[\s\S]*?<\/a>)|(?<=(?:^|[^\\])(?:\\\\)*)&lt;br(?:[ \t](?:(?!&gt;).)*?)?\/?&gt;/giu,
      (match, link?: string) => link ?? "<br>")
